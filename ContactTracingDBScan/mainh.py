...
dic = {'unique_id': all_id, 'datetime': all_date_int, 'room': all_time_int, 'name': all_name}
df = pd.DataFrame(dic)
connection.close()
...
def get_infected_names(input_name):
    ...
    name_room = df[df['unique_id'] == int(input_name)]['room'].item()
    print(name_room)
    epsilon = 1
    ...
    for cluster in input_name_clusters:
        if cluster != -1:
            ids_in_cluster = df.loc[df['cluster'] == cluster, 'unique_id']
    ...
    for i in range(len(infected_unique_names)):
        if df[df['unique_id'] == int(infected_unique_names[i])]['room'].item() == name_room:
            final_infected_names.append(df[df['unique_id'] == infected_unique_names[i]]['name'].item())
